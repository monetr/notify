...
In testing this against a cold tab on slow 3G, `prefetch='idle'` is invisible to users and removes the brief blank gap before the first toast appears.

## SSR

The lazy renderer is gated on `active`, which only flips after a client-side `enqueueSnackbar`. The server output is just `{children}` plus an empty provider — no portals, no `document` access, no hydration mismatch.
